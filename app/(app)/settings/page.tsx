...
export default async function SettingsPage({
  ...
  const { google_calendar_connected, google_calendar_error, gmail_connected, gmail_error } =
    await searchParams;
  const [preferences, shareSettings, googleConnected, gmailConnected, hevyConnected, hevyLastSync] =
    await Promise.all([
  ...
  const gmailNotice = gmail_connected
    ? ({ type: "connected" } as const)
    : gmail_error
      ? ({ type: "error", message: gmail_error } as const)
      : undefined;
  ...
  return (
    <div className="max-w-lg space-y-6">
      ...
      <GmailSettings connected={gmailConnected} notice={gmailNotice} />
      <HevySettings
